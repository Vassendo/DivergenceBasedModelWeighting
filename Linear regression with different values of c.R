#This is the code used to conduct the robustness simulation in Section E of the supplementary material: 
...
run <- foreach(trials = 1:n_trials) %dorng% {
  ...
  for(number in 1:13){
    ...
    model_weights1 <- divergence_weights(pointwise = pointwise, prior = prior, c = 1)
    
    #Calculate stacking weights
    model_weights2 <- divergence_weights(pointwise = pointwise, prior = prior, c = 0.5)
    #model_weights2 <- loo::stacking_weights(lpd_point = lo_matrix)
    
    #Calculate stacking weights
    model_weights3 <- divergence_weights(pointwise = pointwise, prior = prior, c = 2)
    #model_weights2 <- loo::stacking_weights(lpd_point = lo_matrix)
